'''
This files contains utility functions which helps to get prediction from the Object Detection Model
'''


import numpy as np
from objectDetector import net
from objectDetector import weights_loader
import cv2
import warnings
...
def preprocessing(input_img_path,input_height,input_width):
  ...
  input_image = cv2.cvtColor(cv2.imread(input_img_path), cv2.COLOR_BGR2RGB)
...
def postprocessing(predictions,input_img_path,score_threshold,iou_threshold,input_height,input_width):
    ...
    colors = [(254.0, 254.0, 254), (239.88888888888889, 211.66666666666669, 127),
              (225.77777777777777, 169.33333333333334, 100), (211.66666666666669, 127.0, 254),
              (197.55555555555557, 184.66666666666667, 127), (183.44444444444443, 142.33333333333332, 100),
              (169.33333333333334, 100.0, 254), (155.22222222222223, 142.33333333333335, 127),
              (141.11111111111111, 184.66666666666664, 100), (127.0, 254.0, 254),
              (112.88888888888889, 211.66666666666669, 127), (198.77777777777777, 169.33333333333334, 100),
              (184.66666666666667, 127.0, 254), (170.55555555555556, 184.66666666666667, 127),
              (220, 180, 240), (142.33333333333332, 100.0, 254),
              (128.222222222222236, 142.33333333333335, 127), (114.111111111111118, 184.66666666666664, 100),
              (100.0, 254.0, 254), (114.111111111111118, 211.66666666666669, 127)]
    ...
    print(len(thresholded_predictions), thresholded_predictions)

    # if any(thresholded_predictions) == False:
    #     return (0,0)
    ...
    for i in range(len(nms_predictions)):
        ...
        cv2.putText(input_image, str(str(best_class_name) + ' - ' + str(round(thresholded_predictions[i][1], 2))), (int((nms_predictions[i][0][0]+nms_predictions[i][0][2])/2),int((nms_predictions[i][0][1]+nms_predictions[i][0][3])/2)),cv2.FONT_HERSHEY_SIMPLEX,0.5,color,2 )
...
ckpt_folder_path = 'objectDetector/ckpt/'
...
weights_path = 'objectDetector/weights/yolov2-tiny-voc.weights'
...
def predict(input_img_path):
    ...
    output_image_path = 'static/results/'+input_img_path.split('/')[-1]
    ...
    return (output_image_path, output_image)
